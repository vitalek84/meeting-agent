...
class GoogleAccount(BaseModel):
    ...
    name: Optional[str] = Field(default=None, description="Account full name. Useful for an agent for understanding which person in the meeting is an agent")
...
class Settings(BaseSettings):
    ...
    google_accounts: List[GoogleAccount] = Field(default_factory=list, description="List of Google accounts")
    ...
    browser_profile_relative: str = Field(
        ...
        description="Path to the browser profiles directory. Defaults to 'browser_profiles' in the project root."
    )
    ...
    gemini_api_key: str = Field(...,
                                description="Key for AI Agents"
                                )

    wireplumber_cache_src: Path = Field(default=BASE_DIR / "wireplumber" / "restore-stream")
    wireplumber_cache_dst: Path = Field(default=Path("/root/.local/state/wireplumber/restore-stream"))

    technical_screenshots: Path = Field(default=Path("/tmp") / "technical_screenshots", description="GMPageParserAI can save screenshots with debug information."
                                                                                                    " If we enable debug for this module. Screenshots will save to "
                                                                                                    "this folder on the host machine")

    docker_image: str = Field(default="meeting-bot-v2",
                              description="Google meet docker image"
