...
from meeting_agent.gm_helper import ControlFinder, GMPageParserAIv3, ScreenActions
...
class MeetControllerBase:
    def __init__(
        ...
        driver,
        glogin_automation,
        gm_parser: GMPageParserAIv3,
        settings: Settings,
        gm_link: str = None,
        restart_tries=7,
        wait_approval_tries=7,
        meeting_host=False,
    ):
        ...
        if self.settings.user_id:
            try:
                meet_progress = MeetingProgress(
                    ...
                    status=status,
                    gm_link=self.gm_link
                    if status == StatusEnum.meeting_ready
                    else None,
                )
                # StatusEnum.waiting_for_approve
                response = requests.post(
                    settings.callback_url, json=meet_progress.model_dump()
                )
                if response.status_code != 200:
                    self.logger.warning(f"Got error from callback_url: {response.text}")
            except Exception as ex:
                self.logger.info(
                    f"Can't notify connection management API. Error: {ex!s}"
                )
...
class MeetConnectionController(MeetControllerBase):
    async def run(self):
        self.logger.info(
            f"Starting MeetController. Google Account: {self.settings.google_email}"
        )
        if self.gm_link:
...
class MeetState:
    def __init__(
        self,
        controller: Optional[MeetInMeetingController | MeetConnectionController],
        retry: int = 0,
    ):
        self.logger = logging.getLogger(self.__class__.__name__)
...
class MeetingSubMachineInMeeting(MeetState):
    ...
    async def admit_participant(self, finder: ControlFinder) -> Optional[bool | None]:
        ...
        if result:
            ...
            return True
        element, confidence = finder.find_element(
            "someone_wants_to_join_this_call_admit_button"
        )
        if element and confidence >= 80.00:
            self.logger.info(
                "Found someone_wants_to_join_this_call_admit_button. Pressing admit button"
            )
        ...
        element, confidence = finder.find_element(
            ...
            aliases=[
                "meet_call_controls_view_all_participants",
        ...
        if element and confidence >= 60.00:
            self.logger.info(
                "Found that someone wants to join the call. We need to open people popup"
            )
            ScreenActions.click(element)
            await asyncio.sleep(1)
        ...
        if element and confidence > 66.00:
            self.logger.info(
                "People popup: found that someone wants to join the call. Pressing admit personal"
            )
            ScreenActions.click(element)
        ...
        if element and confidence > 75.00:
            self.logger.info(
                "People popup: found that someone wants to join the call. Pressing people_popup_admit_all_button"
            )
            ScreenActions.click(element)
            await asyncio.sleep(1)
            result = ScreenActions.click_icon_with_shift("cancel_admit.png", 40)
        ...
        if confidence == 100.00:
            self.logger.info(
                "Admit all: Found that someone wants to join the call. Pressing admit_all_admit_button"
            )
            ScreenActions.click(element)
    ...
    async def leave_call(self, finder: ControlFinder) -> Optional[bool | None]:
        element, confidence = finder.find_element(
            ...
            aliases=["meet_leave_call_button", "meet_call_control_leave_call_button"],
        )
        if confidence >= 70.00:
    ...
    async def enter(self):
        ...
        try:
            while True:
                self.controller.cur_page = await self.controller.gm_parser.run()
                if (
                    self.controller.cur_page.state
                    == GoogleMeetState.google_meet_meeting_page
                ):
                    ...
                    if self.controller.cur_page.alone_in_the_call:
                        ...
                        if alone_in_the_call_timer >= 20:
                            ...
                            await self.leave_call(finder)
                            self.logger.info(
                                "I am too long alone in this call! Quitting"
                            )
                            break
                    ...
                    self.controller.cur_page.state
                    == GoogleMeetState.google_meet_meeting_connection_page_getting_ready
                ):
                    ...
                    if getting_ready_error_count > 2:
                        self.logger.error(
                            "We saw google_meet_meeting_connection_page_getting_ready more than 3 times. "
                            "It seems there are some issues with a call "
                        )
                        break
                    self.logger.warning(
                        "It seems we are in the call but google_meet_meeting_connection_page_getting_ready "
                        "was detected"
                    )
            ...
            self.logger.error(
                f"Something went wrong in the MeetingSubMachineInMeeting state. Error: {ex}"
            )
...
class PermitMicrophone(MeetState):
    async def enter(self):
        ...
        if (
            self.controller.cur_page.state
            == GoogleMeetState.google_meet_allow_microphone
        ):
            for elem in self.controller.cur_page.elements:
                if elem.label in (
                    PageControls.use_microphone_and_camera.value,
                    PageControls.allow_while_visiting_the_site.value,
                ):
        ...
        elif self.controller.gm_link:
            await self.controller.set_state(
                JoinCurrentMeetingState(self.controller)
            )
        else:
    ...
    async def enter(self):
        ...
        if self.controller.cur_page.state == GoogleMeetState.google_meet_meeting_page:
            ...
            if self.retry < self.MAX_RETRIES:
                ...
                await asyncio.sleep(2)
                await self.controller.set_state(
                    InMeeting(self.controller, retry=self.retry)
                )
            else:
                self.controller.error_msg = (
                    "Current state is google_meet_meeting_connection_page_getting_ready "
            ...
            await self.controller.set_state(
                PermitMicrophone(controller=self.controller)
            )
            ...
            await self.controller.set_state(
                LandingPageState(controller=self.controller)
            )
...
class GoogleLoginState(MeetState):
    ...
    async def enter(self):
        ...
        if (
            self.controller.cur_page.state
            == GoogleMeetState.google_chrome_workspace_popup
        ):
            for elem in self.controller.cur_page.elements:
                if elem.label == PageControls.cancel.value:
                    ...
                    if self.controller.gm_link:
                        ...
                        await self.controller.set_state(
                            LandingPageState(self.controller)
                        )
                    break
            else:
...
class JoinCurrentMeetingAwaitApprovalState(MeetState):
    async def enter(self):
        ...
        if (
            self.controller.cur_page.state
            == GoogleMeetState.google_meet_awaiting_approval_page
        ):
            await asyncio.sleep(3)
            self.controller.wait_approval_tries -= 1
            await self.controller.set_state(
                JoinCurrentMeetingAwaitApprovalState(self.controller)
            )
        elif (
            self.controller.cur_page.state
            == GoogleMeetState.google_meet_allow_microphone
            ...
            await self.controller.set_state(
                PermitMicrophone(self.controller, retry=self.retry)
            )
        elif (
            self.controller.cur_page.state
            == GoogleMeetState.google_meet_meeting_connection_page
        ):
            await asyncio.sleep(3)
            self.controller.wait_approval_tries -= 1
            await self.controller.set_state(
    ...
    async def enter(self):
        ...
        if self.controller.restart_tries == 0:
            ...
            if self.retry < self.MAX_RETRIES:
                ...
                await asyncio.sleep(2)
                await self.controller.set_state(
                    JoinCurrentMeetingState(self.controller, retry=self.retry)
                )
            else:
                self.logger.warning(
                    "Something went wrong we hang in "
            ...
            self.logger.debug(
                f"We are in unknow state (page): {self.controller.cur_page}"
            )
            await self.controller.set_state(JoinCurrentMeetingState(self.controller))
        else:
            self.logger.warning(
                "Can't find proper new state. Retrying to connect to the meeting"
            )
            await self.controller.set_state(JoinCurrentMeetingState(self.controller))
    ...
    async def enter(self):
        ...
        if self.controller.restart_tries == 0:
            ...
            if self.controller.gm_link:
                ...
                self.logger.info("Creating new meeting")
                for elem in self.controller.cur_page.elements:
                    if elem.label == PageControls.new_meeting_button.value:
                        ScreenActions.click(elem)
                        await self.controller.set_state(
                            CreateNewMeetingState(self.controller)
                        )
                        break
                else:
                    await self.controller.set_state(LandingPageState(self.controller))
